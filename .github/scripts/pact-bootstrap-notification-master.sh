...
WORKTREE="${PACT_MASTER_WORKTREE:-/tmp/pact-notification-master-worktree}"
...
(
  ...
  mvn -q verify -Pintegration-tests \
    -Dit.test=TaskEventsPactProviderTest \
    -Djacoco.skip=true \
...
echo "Bootstrapped ${PACT_MAIN_BRANCH} notification-service contracts at ${MASTER_SHA}"
