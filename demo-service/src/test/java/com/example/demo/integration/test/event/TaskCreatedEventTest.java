...
public class TaskCreatedEventTest extends ApiIntegrationTestBase {
    ...
    @Test
    void shouldPublishEventToKafkaTopicTaskEventWhenTaskCreated() {
        // given
        ...
        await().untilAsserted(() -> {
            TaskEvent event = taskEventConsumer.getRecords().stream()
                    .filter(e -> context.getEventType().equals(e.getEventType()))
                    .findFirst()
                    .orElse(null);
